...
function AboutPage({data}) {
	...
	<PageContainer css={baseGrid}>
		...
		<PageSplash bgColor='yellow' color='off-black'>
			...
			<PageTableOfContents>
				{data.attributes.PageSections.map(n => <PageTOCListItem key={n.id}><PageTOCLink href={"#"+n.SectionHeader.replace(/\s+/g, '-').toLowerCase()}>{n.SectionHeader}</PageTOCLink></PageTOCListItem>)}
			</PageTableOfContents>
		</PageSplash>
		<PageIntro>
...

		{
			data.attributes.PageSections.map(
				n => (	<PageSection key={n.id} id={n.SectionHeader.replace(/\s+/g, '-').toLowerCase()} css={baseGrid} isLightSection={n.isLightSection}>

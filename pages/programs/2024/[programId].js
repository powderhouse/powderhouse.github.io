...
function ProgramDetailPage({ programCards, faqs }) {
  ...
  // console.log("2024 PageSections", programCard.attributes.PageSections.map(p => p.SectionHeader));
  let regions = [
    <Header
...
export async function getStaticProps({ params: { programId } }) {
  let programCards = await fetchAPI(
    `/program-cards?filters[ProgramId][$eq]=${programId}-2024&populate[PageSections][populate][0]=PageImage&populate[Meta][populate]=*&populate[ProgramOverview][populate]=*`
  );
